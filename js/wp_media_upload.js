...
jQuery( document ).ready( function( $ ) {
    ...
    var file_frame;
    let button;
    jQuery('form').on("click", "button.upload_image", function( event ){
        ...
        file_frame.on( 'close', function() {
            ...
            // button.closest('div').find('[name="edited[]"]').val('true');
        });
        file_frame.on( 'select', function() {
            ...
            wp.media.attachment(attachment.id).fetch().then(function (data) {
                // preloading finished
                // after this you can use your attachment normally

                button.closest('div').find('img.imgPrev').attr("src", wp.media.attachment(attachment.id).get('url'));
                button.closest('div').find('input[name="image[]"]').val( attachment.id ).trigger('change');
                // button.closest('div').find('input').eq(0).trigger('focusout');
                button.closest('.heading_config').closest('.post_variation').find('button.saveVariation').prop('disabled', false);
            });
        });
    ...
    let buttonh;
    jQuery('form .opening_page .upload_image').click(function( event ){
        ...
        // Create the media frame.
        file_frameh = wp.media.frames.file_frameh = wp.media({
            title: 'Select a image to upload',
            button: {
                text: 'Use this image',
            },
        ...
        // When an image is selected, run a callback.
        file_frameh.on( 'close', function() {
            $('.opening_page select').trigger('change');
        });
        file_frameh.on( 'select', function() {
            // We set multiple to false so only get one image from the uploader
            attachment = file_frameh.state().get('selection').first().toJSON();
            // alert(attachment.id);
            ...
            wp.media.attachment(attachment.id).fetch().then(function (data) {
                ...
                buttonh.closest('div').find('img.imgPrev').attr("src", wp.media.attachment(attachment.id).get('url'));
                buttonh.closest('div').find('input[name="wp_image_id"]').val( attachment.id ).trigger('change');
                jQuery(".in_dash form").trigger("validate_imgTop");
                // jQuery($(this).closest('div').find('input').eq(0).trigger('focusout'));
...
// SECOND ROW
    ...
    // var wp_media_post_id = wp.media.model.settings.post.id;
    let buttonhb;
    jQuery('form .opening_page .img_default').click(function( event ){
        ...
        // Create the media frame.
        file_framehb = wp.media.frames.file_framehb = wp.media({
            title: 'Select a image to upload',
            button: {
                text: 'Use this image',
            },
        ...
        // When an image is selected, run a callback.
        file_framehb.on( 'close', function() {
            $('.opening_page select').trigger('change');
        });
        file_framehb.on( 'select', function() {
            // We set multiple to false so only get one image from the uploader
            attachment = file_framehb.state().get('selection').first().toJSON();
            // alert(attachment.id);
            ...
            wp.media.attachment(attachment.id).fetch().then(function (data) {
                ...
                buttonhb.closest('div').find('img.imgPrevDefault').attr("src", wp.media.attachment(attachment.id).get('url'));
                buttonhb.closest('div').find('input[name="img_default"]').val( attachment.id ).trigger('change');
                // jQuery(".in_dash form").trigger("validate_imgTop");
                // jQuery($(this).closest('div').find('input').eq(0).trigger('focusout'));
...
//    MULTIPLE
    var file_frameMultiple;
    // var wp_media_post_id = wp.media.model.settings.post.id;
    let buttonMultiple;
    jQuery('form .baseBar .multiUpload').click(function( event ){

        buttonMultiple = $(this);
        event.preventDefault();
        // If the media frame already exists, reopen it.
        if ( file_frameMultiple ) {
            // Set the post ID to what we want
            // Open frame
            file_frameMultiple.open();
            return;
        } else {
            // Set the wp.media post id so the uploader grabs the ID we want when initialised
            wp.media.model.settings.post.id = 0;
        ...
        // Create the media frame.
        file_frameMultiple = wp.media.frames.file_frameMultiple = wp.media({
            title: 'Select a image to upload',
            button: {
        ...
        // When an image is selected, run a callback.
        file_frameMultiple.on( 'close', function() {
            setPostEdited();
        });
        file_frameMultiple.on( 'select', function() {
            // We set multiple to false so only get one image from the uploader
            attachment = file_frameMultiple.state().get('selection').toJSON();
            ...
            // Restore the main post ID
            // wp.media.model.settings.post.id = wp_media_post_id;
                wp.media.attachment(v.id).fetch().then(function (data) {
                    // preloading finished
                    // after this you can use your attachment normally
                    duplicateBasePage(wp.media.attachment(v.id).get('url'), v.id);
                    removeFirstEmptyPage();
                });
            });
        });
        // Finally, open the modal
